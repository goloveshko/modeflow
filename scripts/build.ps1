...
{ $env:UPDATE_URL = "https://raw.githubusercontent.com/goloveshko/ModeFlow/main/metadata/update.json" 
}
if (-not $env:LICENSE_URL)
{ $env:LICENSE_URL = "https://github.com/goloveshko/ModeFlow/blob/main/LICENSE" 
}
if (-not $env:GIT_HOST_URL)
{ $env:GIT_HOST_URL = "https://github.com/goloveshko/ModeFlow" 
}
...
foreach ($Config in $ConfigsToBuild)
{
    ...
    & cmake @CmakeArgs
    if ($LASTEXITCODE -ne 0)
    { Write-Err "CMake configuration failed!"; exit $LASTEXITCODE 
    }
    ...
    if ($Test)
    {
        ...
        Set-Location $RootDir
        if ($TestResult -ne 0)
        { Write-Err "Tests failed!"; exit $TestResult 
        }
    ...
    if ($Package)
    {
        ...
        $RawChangelog = if (Test-Path $ChangelogFile)
